...
function loadPage(page) {
  ...
  // 화면에 처음 보여질 사람의 데이터가 전체 데이터 중에 몇번째인지 특정합니다.
  let startPost = (dataObj.currentPage - 1) * dataObj.datasPerPeage;

  // 총 몇개의 페이지가 만들어져야하는지 특정합니다.
  let totalPages = Math.ceil(dataObj.datas.length / dataObj.datasPerPeage);

  // 페이지마다 화면에 마지막으로 보여질 사람의 데이터가 몇번째인지 특정합니다.
  let endPost =
    // 90 + 10 > 97
    startPost + dataObj.datasPerPeage > dataObj.datas.length
      ? dataObj.datas.length
      : startPost + dataObj.datasPerPeage;
  ...
  for (let i = startPost; i < endPost; i++) {
    ...
    card.innerHTML = `
    <img src=${result.picture.medium} class='img-card' alt=''/>
