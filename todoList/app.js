...
// 할 일 데이터를 localStrage저장하는 함수입니다.
function saveTasks() {
  localStorage.setItem('tasklist', JSON.stringify(tasks));
}

// 초기에 로컬스토리지에 저장된 데이터를 기반으로 li목록 생성하기
if (tasks) {
  tasks.forEach((task) => {
...
// 할일 목록 데이터를 업데이트 하는 함수입니다.
function buildTasks() {
  // 테스크 목록의 정보를 업데이트 하기 위해서 기존 데이터를 초기화합니다.
  tasks.length = 0;
...
function downloadFile() {
  ...
  curList.forEach((el) => {
    ...
    element.setAttribute('href', 'data:text/plain;chrset=utf-8,' + temp);
